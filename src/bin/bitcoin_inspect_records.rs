...
fn main() {
    println!("Inspecting contents of vault records...");
                    ...
                    if data.len() > 88 {
                        ...
                        for r in 0..record_count {
                            ...
                            if pos + record_len <= vault_payload.len() {
                                ...
                                // Print records that contain readable ASCII or non-trivial data
                                if record_len > 10 {
                                    let text_lossy = String::from_utf8_lossy(record_data);
                                    let printable_count = text_lossy.chars().filter(|c| c.is_ascii_alphanumeric() || c.is_ascii_punctuation()).count();
                                    
                                    if printable_count as f32 / record_len as f32 > 0.6 {
                                        println!("  [Record #{}] (Len: {}) Text: {}", r, record_len, text_lossy);
                                    } else {
                                        println!("  [Record #{}] (Len: {}) Hex preview: {:02x?}", r, record_len, &record_data[..record_data.len().min(16)]);
                                    }
                                }
    ...
    println!("\nVault record inspection complete.");
}
