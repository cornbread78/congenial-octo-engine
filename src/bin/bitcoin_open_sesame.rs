...
fn main() {
    println!("Unsealing vault records using the 'open sesame' key...");
            ...
            if name == "blk05673_decrypted.dat" {
                if let Ok(data) = fs::read(entry.path()) {
                    if data.len() > 88 {
                        ...
                        for r in 0..record_count {
                            ...
                            if pos + record_len <= vault_payload.len() {
                                ...
                                for (i, byte) in record_data.iter_mut().enumerate() {
                                    *byte ^= sesame_key[i % sesame_key.len()];
                                }
                                ...
                                if printable as f32 / record_data.len() as f32 > 0.7 {
                                    println!("  [Record #{}] (Len: {}) Decrypted Text: {}", r, record_len, text_lossy);
                                    readable_count += 1;
                                } else if r < 15 {
                                    println!("  [Record #{}] (Len: {}) Hex preview: {:02x?}", r, record_len, &record_data[..record_data.len().min(12)]);
                                }
                        ...
                        println!("\nUnsealing complete. Successfully recovered readable records: {}", readable_count);
                    }
                }
            }
        }
