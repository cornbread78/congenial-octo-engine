...
fn main() {
    println!("Deriving keys from 'Alpha Root Kernel' and string path b\"04/04/00/00\"...");
    let blocks_path = Path::new("./datadir/blocks");
    
    let derived_key = hmac_sha256(b"Alpha Root Kernel", b"04/04/00/00");
            ...
            if name == "blk05673_decrypted.dat" {
                if let Ok(data) = fs::read(entry.path()) {
                    if data.len() > 88 {
                        ...
                        for r in 0..record_count {
                            ...
                            if pos + record_len <= vault_payload.len() {
                                ...
                                if printable as f32 / record_data.len() as f32 > 0.65 {
                                    ...
                                    println!("  [Record #{}] (Len: {}) Hex: {:02x?}", r, record_len, &record_data[..record_data.len().min(12)]);
                                }
                        ...
                        println!("\nString path derivation decoding complete. Readable records: {}", readable_count);
                    }
                }
            }
        }
