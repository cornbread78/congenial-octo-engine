...
fn test_alibaba_variant(vault_payload: &[u8], sub_data: &[u8], label: &str) {
...
fn main() {
    println!("Testing Ali Baba middle key variants...");
            ...
            if name == "blk05673_decrypted.dat" {
                if let Ok(data) = fs::read(entry.path()) {
                    if data.len() > 88 {
                        ...
                        test_alibaba_variant(&vault_payload, b"open sesame:Ali Baba:close sesame", "open:alibaba:close");
                        test_alibaba_variant(&vault_payload, b"open sesame/Ali Baba/close sesame", "open/alibaba/close");
                        test_alibaba_variant(&vault_payload, b"open sesame Ali Baba close sesame", "open_space_alibaba_space_close");
                        test_alibaba_variant(&vault_payload, b"Ali Baba", "alibaba_only");
                        test_alibaba_variant(&vault_payload, b"open sesame:alibaba:close sesame", "open:alibaba_lower:close");
                    }
                }
            }
        }
