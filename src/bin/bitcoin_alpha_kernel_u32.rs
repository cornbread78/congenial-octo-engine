...
fn test_derivation(path_bytes: &[u8], label: &str) {
    println!("--- Testing {} Path Derivation ---", label);
    let derived_key = hmac_sha256(b"Alpha Root Kernel", path_bytes);
    println!("  [Derived Key] {:02x?}", &derived_key[..16]);
            ...
            if name == "blk05673_decrypted.dat" {
                if let Ok(data) = fs::read(entry.path()) {
                    if data.len() > 88 {
                        ...
                        println!("  Readable records found: {}\n", readable_count);
                    }
                }
            }
        }
...
fn main() {
    let path_vals: [u32; 4] = [4, 4, 0, 0];
    
    // Big-Endian u32 serialization
    let mut be_bytes = Vec::new();
    for &val in &path_vals {
        be_bytes.extend_from_slice(&val.to_be_bytes());
    }
    test_derivation(&be_bytes, "Big-Endian u32");
    
    // Little-Endian u32 serialization
    let mut le_bytes = Vec::new();
    for &val in &path_vals {
        le_bytes.extend_from_slice(&val.to_le_bytes());
    }
    test_derivation(&le_bytes, "Little-Endian u32");
}
